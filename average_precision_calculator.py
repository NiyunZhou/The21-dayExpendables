...
class AveragePrecisionCalculator(object):
  ...
  def peek_ap_at_n_out(self):
    ...
    ap, output_data = self.ap_at_n_out(predlists[0],
                      predlists[1],
                      n=self._top_n,
                      total_num_positives=self._total_positives)
    return ap, output_data
  ...
  @staticmethod
  def ap_at_n_out(predictions, actuals, n=20, total_num_positives=None):
    """Calculate the non-interpolated average precision.
    ...
    data_output = {}
    data_output["confidence"] = []
    data_output["ap"] = []
    data_output["weight_i"] = []
    ...
    for i in range(r):
      if actuals[sortidx[i]] > 0:
        poscount += 1
        ap += poscount / (i + 1) * delta_recall
        data_output["confidence"].append(predictions[sortidx[i]])
        data_output["ap"].append(ap)
        data_output["weight_i"].append(i)
    return ap, data_output
