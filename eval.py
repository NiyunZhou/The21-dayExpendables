...
if __name__ == "__main__":
  ####################################################################################################################
  #By Dalong:同样，这个文件我们也只需要管这几个宏定义变量的合适设置即可。
  #大部分的参数设置与我上传在github上的train.py文件里的描述相同。这里只写之前未
  #提及的参数设置：
  #train-dir:模型文件的存放地址。和train.py里的模型保存路径要保持一致
  #eval_data_pattern:要测试的数据文件路径
  #By  Dalong:同样在这个文件中我遇到了几个问题。分别注释在了第233行和第308行
  #如果要想读这个代码架构，只需要看最后一个evaluate()函数即可，我给那里面每一个部分都加了详细的注释，标注他是在干什么
  ###################################################################################################################
  # Dataset flags.
...
###########################################################################
#By  Dalong:这里是找到你所提供的文件列表(modules)中所有的
#名称为name的class。这个我们可以不用管。因为modules，默认的是
#[frame_level_models, video_level_models]，然后name就是我们选择的模型的名称
#比如：LogisticModel
############################################################################
...
##########################################################################################
#By Dalong:功能如名称，获得待测试的数据的一个batch
#返回值至少包含（features，labels)但是具体的维度和你要读取的数据有关
#比如如果你要读取视频数据则返回(video_id_batch, model_input_raw, labels_batch, num_frames)
##########################################################################################
...
def build_graph(reader,
  ...
  ###########################################################
  #By  Dalong:把这些变量进行全局存储，可以在任何地方进行访问
  #
  ###########################################################
...
def evaluate():
  ...
  with tf.Graph().as_default():
	...
	###############################################################
	#By Dalong：不知道为什么feature_names会是一个列表
	###############################################################
    feature_names, feature_sizes = utils.GetListOfFeatureNamesAndSizes(
        FLAGS.feature_names, FLAGS.feature_sizes)
	######################################################
	#By Dalong:这里对相应的文件生成了对应的读取器
	#####################################################
    ...
    #############################################################
	#By Dalong:这里初始化了一个用来计算loss的类。主要的参数就是
	#总共的类别（在我们这个比赛中总共4076个类）/对每一个video，给出多少个预测值
	#############################################################
	...
	##########################################################
	#By Dalong:这个循环需要我们自己改么？不然循环是怎么结束的？
	##########################################################
    while True:
